...
import { isBoolExp, isNumExp, isPrimOp, isStrExp, isVarDecl, isVarRef, isSetExp,
    isAppExp, isDefineExp, isIfExp, isLetExp, isProcExp, isProgram,
    parseL21, unparse, parseL21Exp, Exp } from '../src/ex3/L21-ast';
import { Result, bind, isOkT, makeOk, isFailure } from '../src/shared/result';
import { parse as parseSexp } from "../src/shared/parser";

const p = (x: string): Result<Exp> => bind(parseSexp(x), parseL21Exp);
...
describe('L21 Parser', () => {
    ...
    it('parses programs', () => {
        expect(parseL21("(L21 (define x 1) (> (+ x 1) (* x x)))")).to.satisfy(isOkT(isProgram));
    });
    ...
    describe("Failures", () => {
        it("returns a Failure when parsing a single-token program", () => {
            expect(parseL21("x")).to.satisfy(isFailure);
        });

        it("returns a Failure when parsing an empty program", () => {
            expect(parseL21("")).to.satisfy(isFailure);
        });

        it("returns a Failure if the program does not start with (L3 ...)", () => {
            expect(parseL21("(+ 1 2)")).to.satisfy(isFailure);
        });
...
describe('L21 Unparse', () => {
    ...
    it('unparses programs', () => {
        const program = "(L21 (define x 1) (> (+ x 1) (* x x)))";
